    ...
    <div class="container container-md">
        ...
        <table class="table table-bordered">
            ...
            <tbody class="table-group-divider">
                ...
                LEFT JOIN user u2 ON r.user_id = u2.id WHERE status = 'Completed'
                UNION DISTINCT
                SELECT n.notice_id notice_id, u2.nick_name c_nick_name, u1.nick_name r_nick_name, u2.icon c_icon, u1.icon r_icon, u1.email r_email, message, substr(response_date_time, 1, 16) response_date_time, title, n.type type, status, substr(date_time, 1, 16) date_time, venue, description, n.email email, phone, image, substr(post_date_time, 1, 16) post_date_time 
                FROM user u1 
                LEFT JOIN response r ON u1.id = r.user_id 
                LEFT JOIN notice n ON r.notice_id = n.notice_id 
                LEFT JOIN user u2 ON n.user_id = u2.id WHERE status = 'Completed') ORDER BY post_date_time DESC";
